...
describe('cms.releases.candidate-checklist', () => {
  it('surfaces validation and workflow readiness separately for a fresh draft', () => {
    ...
    expect(checklist?.items.find(item => item.id === 'content_qa')?.status).toBe('ready')
  })
  ...
  it('surfaces accessibility and editorial QA findings separately from content integrity', () => {
    const settings = createDefaultWhiteLabelSettings()
    settings.governance.workflow.status = 'approved'
    ...
    expect(checklist).not.toBeNull()
    expect(checklist?.items.find(item => item.id === 'content_integrity')?.status).toBe('ready')
    expect(checklist?.items.find(item => item.id === 'content_qa')?.status).toBe('warning')
    expect(checklist?.items.find(item => item.id === 'content_qa')?.issues.map(issue => issue.code)).toEqual(
      expect.arrayContaining(['quality.page.description.missing', 'a11y.media.alt.missing'])
    )
  })
})