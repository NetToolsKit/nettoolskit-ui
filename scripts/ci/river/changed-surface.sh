...
requires_gate() {
  ...
  case "$gate_name" in
    prepare)
      case "$checked_path" in
        package.json | package-lock.json | sources.lock.yaml | index.ts | tsconfig.json | tsup.config.ts | vite.config.ts | playwright.config.ts | eslint.config.mjs | stylelint.config.mjs | \
    ...
    standard)
      case "$checked_path" in
        README.md | DESIGN.md | COMPONENTS.md | TOKENS.md | CHANGELOG.md | package.json | package-lock.json | sources.lock.yaml | index.ts | tsconfig.json | tsup.config.ts | vite.config.ts | playwright.config.ts | eslint.config.mjs | stylelint.config.mjs | \
          src/* | src/** | samples/* | samples/** | tests/* | tests/** | scripts/* | scripts/** | policies/* | policies/** | .gitriver/* | .gitriver/** | .github/* | .github/**)
          return 0
          ;;
      esac
    ...
    test)
      case "$checked_path" in
        package.json | package-lock.json | sources.lock.yaml | index.ts | tsconfig.json | vite.config.ts | playwright.config.ts | \
          src/* | src/** | samples/* | samples/** | tests/* | tests/** | scripts/* | scripts/** | policies/* | policies/** | .gitriver/* | .gitriver/** | .github/* | .github/**)
          return 0
          ;;
      esac
